<script type="module">
  ...
  function parseRowsFromLines(lines, pageNumber) {
    ...
    for (const line of lines) {
      ...
      // DABC - DAOR 6169 B738 7T-VKA
      const routeFlightRegPattern = /([A-Z]{3,4}\s*-\s*[A-Z]{3,4})\s+(\d{3,4})\s+[A-Z0-9]{3,4}\s+(7T-[A-Z0-9]{3})/g;
      let match;
  ...
  extractBtn.addEventListener('click', async () => {
    try {
      ...
      if (result.rows.length) {
        setStatus(`Done. Found ${result.rows.length} flight record(s). Routes are displayed in ICAO and flights as DAHXXXX.`);
      } else {
        setStatus('PDF was read, but no matching flight rows were found. Check the text preview to adjust the regex for your PDF format.', true);
      }
    } catch (error) {
      console.error(error);
